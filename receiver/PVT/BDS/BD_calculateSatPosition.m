function [satPositions, satClkCorr, eph] = BD_calculateSatPosition(transmitTime, ephemeris, activeChannel)
%星历参数
...
satClkCorr=zeros(2,32);
for i = 1:length(activeChannel)
    prnList(i) = activeChannel(i);
    eph(prnList(i)) = ephemeris(activeChannel(i)).eph;
end
...
for satNr = 1 : numOfSatellites
    ...
    omegaDot=eph(prn).omegaDot;%OMEGA_DOT%升交点经度变化率
    ...
    satPositions(4, prn) = velocity(1);
    satPositions(5, prn) = velocity(2);
    satPositions(6, prn) = velocity(3);
    ...
    dtr = F*e*sqrtA * sin(E);
    satClkCorr(1,prn)=a0+(a1+a2*dt)*dt+dtr-TGD1;        % 卫星钟差
    dtr_dot = F*e*sqrtA * E1 * cos(E);
    satClkCorr(2,prn) = a1 + 2*a2*dt + dtr_dot;         % 钟差漂移
end
end
