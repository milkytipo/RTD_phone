function [satPositions, satClkCorr] = bdsb1i_calc_OneSatPosition(transmitTime, eph, prn)
...
satPositions(4) = velocity(1);
satPositions(5) = velocity(2);
satPositions(6) = velocity(3);
...
dtr = F*e*sqrtA * sin(E);
satClkCorr(1) = a0+(a1+a2*dt)*dt+dtr-TGD1;        % 卫星钟差
dtr_dot = F*e*sqrtA * E1 * cos(E);
satClkCorr(2) = a1 + 2*a2*dt + dtr_dot;         % 钟差漂移
