function [pvtCalculator, recv_timer, satelliteTable_bds, satelliteTable_gps] = ...
    kalmanPVT_resiraim_JointBdsGps(satpos_bds, satpos_gps, obs_bds, obs_gps, transmitTime_bds, transmitTime_gps, satClkCorr_bds, satClkCorr_gps, ...
                       CN0_bds, CN0_gps, ...
...
while (nmbOfSat_inraim>=5) && (inraim == 1) && (NGEOFlag>0)
    if pvtCalculator.kalman.preTag == 1 % Kalman filter has been properly initialized, the KalmanPos can be performed
        if nmbOfSat_inraim_gps==0 %there is only BDS sats available
            [pos_xyz, vel_xyz_bds, cdtu_bds, az_actv_bds, el_actv_bds, iono_actv_bds, trop_actv_bds, bEsti_bds, psrCorr_bds, DOP, KalFilt_tmp] = ...
                kalmanPos_BDS1(satpos_bds, ...
                               obs_bds, ...
                               transmitTime_bds, ...
                               activChn_raim_bds, ...
                               satClkCorr_bds, ...
             ...
             bEsti = bEsti_bds;
             vel_xyz = [vel_xyz_bds; KalFilt_tmp.stt_dtf(2,2)]; % For GPS clk drift, we keep it unchanged
             cdtu = [cdtu_bds; KalFilt_tmp.stt_dtf(1,2)]; % we use the predicted clkErr as cdtu of GPS
             rankBreak = 0;
        elseif nmbOfSat_inraim_bds==0 %there is only GPS sats available
            [pos_xyz, vel_xyz_gps, cdtu_gps, az_actv_gps, el_actv_gps, iono_actv_gps, trop_actv_gps, bEsti_gps, psrCorr_gps, DOP, KalFilt_tmp] = ...
                kalmanPos_GPS1(satpos_gps, ...
                               obs_gps, ...
                               transmitTime_gps, ...
                               activChn_raim_gps, ...
                               satClkCorr_gps, ...
            ...
            bEsti = bEsti_gps;
            vel_xyz = [vel_xyz_gps(1:3); KalFilt_tmp.stt_dtf(2,1); vel_xyz_gps(4)]; % For BDS clk drift, we keep it unchanged
            cdtu = [KalFilt_tmp.stt_dtf(1,1); cdtu_gps]; % we use the predicted clkErr as cdtu of BDS
            rankBreak = 0;
        else
            [pos_xyz, vel_xyz, cdtu, az_actv_bds, az_actv_gps, el_actv_bds, el_actv_gps, iono_actv_bds, iono_actv_gps, trop_actv_bds, trop_actv_gps, ...
                bEsti, psrCorr, DOP, KalFilt_tmp] = ...
                    kalmanPos_JointBdsGps1(satpos_bds, satpos_gps, ...
                                    obs_bds, obs_gps, ...
            ...
            rankBreak = 0;
        end %EOF "if nmbOfSat_inraim_gps==0"
    else % Perform least-square positioning
        if nmbOfSat_inraim_gps==0 %there is only BDS sats available
            ...
            [pos_xyz, vel_xyz, cdtu, ...
                ...
                bEsti, psrCorr, DOP, rankBreak] = ...
                    leastSquarePos_JointBdsGps1(satpos_bds, satpos_gps, ...
                                    ...
                                    CN0_bds, CN0_gps);
            psr = [psr_bds(activChn_raim_bds(2,:)), psr_gps(activChn_raim_gps(2,:))];
        end %EOF "if nmbOfSat_inraim_gps==0 %there is only BDS sats available"
    end %EOF "if pvtCalculator.kalman.preTag == 1"
...
%         pvtCalculator.clkErr(2,2) = pvtCalculator.clkErr(2,2) + vel_xyz(5);  % GPS: accumulated clk drift freq [Hz]
        ...
        switch pvtCalculator.kalman.preTag
            case 0 % The Kalman filter has not been initialized
                % Both BDS and GPS have been involved in the PVT
                if (recv_timer.rclkErr2Syst_UpCnt(1) + recv_timer.rclkErr2Syst_UpCnt(2)) == 0
                    pvtCalculator.kalman.preTag = 2; % Kalman filter is ready for being initialized
                end
            case 1
                pvtCalculator.kalman = KalFilt_tmp;
                pvtCalculator.kalman.stt_dtf(1,1) = 0; % local clk error has been corrected in recv_timer for BDS
                pvtCalculator.kalman.stt_dtf(1,2) = 0; % for GPS
        end
