...
prError_BDS = [];   % BDS与上一时刻定位结果的伪距残差
prError_GPS = [];   % GPS与上一时刻定位结果的伪距残差
%---
T_amb = 20;%20
P_amb = 101.325; %KPa
...
if (length(activeChannel.BDS(1,:))+length(activeChannel.GPS(1,:))) >= 5  
    ...
    for iter = 1:nmbOfIterations
        ...
        for i = 1:nmbOfSatellites.BDS
           ...
           Beta_i.BDS =[ephemeris(1).para(activeChannel.BDS(2,i)).eph.Beta0,ephemeris(1).para(activeChannel.BDS(2,i)).eph.Beta1, ...
            ...
            if iter == 1
                ...
                Rot_X.BDS(:, activeChannel.BDS(2,i)) = sat_xyz.BDS(:, activeChannel.BDS(2,i));%卫星i的位置
                trop_BDS = 4;%电离层与对流层的延时
            else
                %--- Update equations -----------------------------------------
                rho2 = (Rot_X.BDS(1, activeChannel.BDS(2,i)) - pos(1))^2 + (Rot_X.BDS(2, activeChannel.BDS(2,i)) - pos(2))^2 + ...
                    (Rot_X.BDS(3, activeChannel.BDS(2,i)) - pos(3))^2;%卫星i的伪距平方
        ...
        for i = 1 : nmbOfSatellites.GPS
            ...
            if iter == 1
                %--- Initialize variables at the first iteration --------------
                Rot_X.GPS(:, activeChannel.GPS(2,i)) = sat_xyz.GPS(:, activeChannel.GPS(2,i));%卫星i的位置
                trop_GPS = 4;%电离层与对流层的延时
            else
                %--- Update equations -----------------------------------------
                rho2 = (Rot_X.GPS(1, activeChannel.GPS(2,i)) - pos(1))^2 + (Rot_X.GPS(2, activeChannel.GPS(2,i)) - pos(2))^2 + ...
                    (Rot_X.GPS(3, activeChannel.GPS(2,i)) - pos(3))^2;%卫星i的伪距平方
                traveltime = sqrt(rho2) / 299792458 ;
                %--- Correct satellite position (do to earth rotation) --------
        ...
        if iter >= 6
            satUsed.GPS = posiChannel.GPS(2,:);  % 参与位置与速度解算的卫星号
            satUsed.BDS = posiChannel.BDS(2,:);  % 参与位置与速度解算的卫星号
            bEsti = omc - A/(A'*A)*A'*omc;    % 残余分量
            WSSE = (norm(bEsti))^2;         % 误差检测值
            if WSSE < chi2inv(0.99999, size(A,1)-4)     % 如果判断有错误卫星，则不经进行仰角判断
            % 先去除GPS仰角低于elevationMask的卫星    
    ...
    for k = 1:length(satUsed.BDS)
        if pvtCalculator.BDS.doppSmooth(satUsed.BDS(k), 2) ~= 0 
            deltaP = pvtCalculator.BDS.doppSmooth(satUsed.BDS(k),1) - pvtCalculator.BDS.doppSmooth(satUsed.BDS(k),2);   %积分多普勒一秒的变化量（m）
        else
            deltaP = -299792458/1561098000*pvtCalculator.BDS.doppSmooth(satUsed.BDS(k),4);    % 多普勒频移（m）
        end
    ...
    for k = 1:length(satUsed.GPS)
        if pvtCalculator.GPS.doppSmooth(satUsed.GPS(k), 2) ~= 0 
            deltaP = pvtCalculator.GPS.doppSmooth(satUsed.GPS(k),1) - pvtCalculator.GPS.doppSmooth(satUsed.GPS(k),2);   %积分多普勒一秒的变化量（m）
        else
            deltaP = -299792458/1575420000*pvtCalculator.GPS.doppSmooth(satUsed.GPS(k),4);    % 多普勒频移（m）
        end
    ...
    for k = 1 : size(posiChannel.BDS, 2)
        ...
        rho2 = (sat_xyz.BDS(1, posiChannel.BDS(2,k)) - posiFore(1))^2 + (sat_xyz.BDS(2, posiChannel.BDS(2,k)) - posiFore(2))^2 + ...
                    (sat_xyz.BDS(3, posiChannel.BDS(2,k)) - posiFore(3))^2;%卫星i的伪距平方
        traveltime = sqrt(rho2) / 299792458 ; 
        %--- Correct satellite position (do to earth rotation) --------
        Rot_X.BDS(:, posiChannel.BDS(2,k)) = e_r_corr(traveltime, sat_xyz.BDS(:, posiChannel.BDS(2,k)));%卫星i经过地球自转修正后的位置
    ...
    clcErrFore = median(prError_BDS);    % 预估出接收机的钟差值
    prError_BDS = prError_BDS - clcErrFore;   % 去除钟差值
    %——————————————计算伪距与上次定位结果的残差  GPS——————————————%
    for k = 1 : size(posiChannel.GPS, 2)
        ...
        rho2 = (sat_xyz.GPS(1, posiChannel.GPS(2,k)) - posiFore(1))^2 + (sat_xyz.GPS(2, posiChannel.GPS(2,k)) - posiFore(2))^2 + ...
                    (sat_xyz.GPS(3, posiChannel.GPS(2,k)) - posiFore(3))^2;%卫星i的伪距平方
        traveltime = sqrt(rho2) / 299792458 ; 
        %--- Correct satellite position (do to earth rotation) --------
        Rot_X.GPS(:, posiChannel.GPS(2,k)) = e_r_corr(traveltime, sat_xyz.GPS(:, posiChannel.GPS(2,k)));%卫星i经过地球自转修正后的位置
    ...
    clcErrFore = median(prError_GPS);    % 预估出接收机的钟差值
    prError_GPS = prError_GPS - clcErrFore;   % 去除钟差值
    prError = [prError_BDS, prError_GPS];
    %——————————————若本次定位结果正确，则记录本次定位结果————————%
    bEsti = omc - A/(A'*A)*A'*omc;    % 残余分量
    WSSE = (norm(bEsti))^2;         % 误差检测值
    if size(A,1) > 5    % 则定位方程为超定方程
        if WSSE < chi2inv(0.99999, size(A,1)-5)     % 判断定位结果是否正确
            pvtCalculator.posiLast = posvel';    % 若正确，记录定位结果
            pvtCalculator.posiTag = 1;  % 位置信息已更新
            pvtCalculator.posiCheck = 1;    % 认为定位结果可信
        end
    else
        if pvtCalculator.posiLast(1)~=0 && pvtCalculator.posiCheck==1  % 有可信结果
    ...
    if pvtCalculator.kalman.preTag==0 && pvtCalculator.posiCheck==1
        pvtCalculator.kalman.state = posvel([1,6,2,7,3,8,4,9,5,10])';     % 状态变量  N-by-1    静态模型只包含位置和钟差
        P_1 = [36,0;0,0.1];
        pvtCalculator.kalman.P = blkdiag(P_1,P_1,P_1,P_1,P_1);         % 误差协方差矩阵
