%% start PVT
% initial
function [recv_time, ephemeris, pvtCalculator, config] = pointGPS(channels, config, recv_time, ephemeris, pvtCalculator) 
%% 初始化
svnum = 0;% this flag is to find whether avaliable satellite is above 4
activeChannel = [];% avaliable channels
inteDoppler = zeros(1,32);%积分多普勒值
dopplerfre = zeros(1,32);%多普勒频移
CNR = zeros(1,32);  %输出信噪比
...
for  n=1:config.numberOfChannels(1).channelNumAll
    ...
    if  ~isnan(prnNum) && (ephemeris(prnNum).ephReady==1 || ephemeris(prnNum).updateReady == 1) ...
            && ephemeris(prnNum).ephUpdate.health==0 && strcmp(channels(n).CH_L1CA(1).CH_STATUS, 'SUBFRAME_SYNCED')        
        svnum = svnum + 1;
        activeChannel(1,svnum) = n;
        activeChannel(2,svnum) = channels(n).CH_L1CA(1).PRNID;
        ...
        % 积分多普勒值
        inteDoppler(activeChannel(2,svnum)) = -1*channels(n).CH_L1CA(1).carrPhaseAccum*299792458/1561098000; 
        % 多普勒频移
        dopplerfre(activeChannel(2,svnum)) = channels(n).CH_L1CA(1).LO2_fd;
        ...
        % 载噪比
        CNR(activeChannel(2,svnum))=channels(n).CH_L1CA(1).CN0_Estimator.CN0;
        % 载波环方差
        carrierVar(activeChannel(2,svnum))=channels(n).CH_L1CA(1).sigma;
        % 信噪比
 ...
 %%
    if n == config.numberOfChannels(1).channelNumAll && svnum>=1 
      ...
      % find trasmition time 
       [transmitTime] = findTransTime_GPS(channels,activeChannel(1,:));
       recv_time.weeknum = ephemeris(activeChannel(2,1)).eph.weekNumber;  %%更新周计数
      % Compute satellite position
        [satPositions, satClkCorr,ephAll] = GPS_calculateSatPosition(transmitTime, ...
      ...
      [rawP] = calculatePseudoranges_GPS(transmitTime,rxTime,activeChannel);
%       fprintf('rawP -- %.6f \n', rawP);
       ...
       fclose(fid);
      % Performing the PVT
       [xyzdt,el,az,dop] = leastSquarePos_GPS(satPositions, rawP+satClkCorr(1,:)*299792458, ... 
           transmitTime-satClkCorr(1,:), ephemeris, activeChannel, config.elevationMask,satClkCorr(2,:),pvtCalculator); %freqforcal,reveiver.recv_cfg);       
        queue=sort(el(2,:));
...
%       enuVel = xyz2enu(xyzdt(5:7), [0,0,0]);
       ...
       [BJyear,BJmonth,BJday] = calculateGPS_yymmdd(recv_time.weeknum, BJday_1);
       ...
       fid_0=fopen(logName);
       if fid_0==-1
           rinexobsGPS_header(xyzdt,recv_time, logName);
       else
           fclose(fid_0);
       end
       rinexobsGPS_data(recv_time,rawP,inteDoppler,dopplerfre,CNR,xyzdt(4),queue, logName);
       ...
       [pvtCalculator] = rinexnaviGPS_data(recv_time,ephAll,satClkCorr(1,:),queue, pvtCalculator,logName);
       
       logName = strcat(config.logFilePath, pvtCalculator.logName, '.15MP');
       fid_2=fopen(logName);
       if fid_2==-1
           rinexmpGPS_header(recv_time,logName);
       else
           fclose(fid_2);
       end
       rinexmpGPS_data(recv_time,channels,activeChannel,logName); 
