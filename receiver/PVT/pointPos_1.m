...
function [receiver] = pointPos_1(receiver, pvtForecast_Succ)
%% Initialization
...
if (svnum.BDS + svnum.GPS) >= 1
    ...
    if svnum.GPS >= 1
        ...
        [satPositions.GPS, satClkCorr.GPS, EphAll.GPS] = GPS_calculateSatPosition(transmitTime.GPS, naviMsg.GPS_L1CA.ephemeris, activeChannel.GPS(2,:));
        % Get the receiver local time
        [rxTime_GPS] = get_rxTime('GPS_L1CA', recv_timer);
    ...
    if svnum.BDS >= 1
        ...
        % Compute satellite position
        [satPositions.BDS, satClkCorr.BDS, EphAll.BDS] = BD_calculateSatPosition(transmitTime.BDS, naviMsg.BDS_B1I.ephemeris, activeChannel.BDS(2,:));
        % Get the receiver local time
        [rxTime_BDS] = get_rxTime('BDS_B1I', recv_timer);
        % Compute the Pseudo-range / receiver time
