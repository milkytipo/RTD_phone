%% Acquire GEO and NGEO COMPASS satllites
function [channel_spc, acqResults] = acquireCompass(channel_spc, sis, acqResults, bpSampling_OddFold, logConfig)
...
%     fprintf('/----------------------------------------------------------------------------------------------/\n');
    fprintf('     %s BDS PRN%d:  Coherent accumulation: %1.3fs ; FreqBin: %dHz ; FreqRange: -%d~+%dHz\n', ...
    ...
    % PRN code
    channel_spc.codeTable = generateGoldCode('BDS_B1I', channel_spc.PRNID);
...
while(1)
    ...
    elseif (acqResults.acqed==0) && (channel_spc.acq.sampPer2TC_s+Samp_Posi<N)
        ...
        [channel_spc, acqResults] = acquireCompass_1ms(logConfig, channel_spc, sis(sis_index), acqResults, bpSampling_OddFold);
    ...
    elseif channel_spc.acq.sampPer2TC_s+Samp_Posi >= N   % 为了防止在总数据边缘处发生采样点跳跃导致出错，此处判断条件设为大于等于
        channel_spc.acq.resiData = sis(Samp_Posi+(1:(N-Samp_Posi)));    
