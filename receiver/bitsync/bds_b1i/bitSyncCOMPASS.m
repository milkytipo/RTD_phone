function [channel_spec, bitSyncResults] = bitSyncCOMPASS(logConfig, channel_spec, sis, bitSyncResults)

if ~size(channel_spec.bitSync.corr)
    fprintf('/----------------------------------------------------------------------------------------------/\n');
    fprintf('BitSyncing BDS PRN%d:  Coherent accumulation: %1.3fs ; Non-coherent: %d ; FreqBin: %dHz ; FreqRange: -%d~+%dHz\n', ...
...
% Samp_Posi = channel_spec.Samp_Posi + round(channel_spec.bitSync.Samp_Posi_dot);              % 取整后的采样点位置
Samp_Posi = channel_spec.Samp_Posi;
...
while 1
    ...
    elseif(bitSyncResults.synced==0)&&(Samp_Posi+channel_spec.bitSync.sampPerCode<N)
        ...
        if channel_spec.bitSync.waitNum <= 0
            [channel_spec, bitSyncResults] = bitSyncCOMPASS_1ms(logConfig, channel_spec, sis(sis_index), bitSyncResults);
        else
