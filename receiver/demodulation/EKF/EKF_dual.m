...
function [Xo,Po] = EKF_dual(Q,R,Z,Xi,Pi,SV,T,posiChannel,useNum_BDS)
N_state = size(Xi, 1);    

[Xp, ~] = ConstantVelocity_dual(Xi, T);%1 状态预测值

[~, fy] = ConstantVelocity_dual(Xp, T);%2 状态转移矩阵

[gXp, H] = obsEquation_dual(Xp, SV,useNum_BDS);%3 观测方程：  gXp：预测观测值   H：观测矩阵
