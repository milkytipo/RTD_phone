...
function [Xo,Po] = EKF_static(Q,R,Z,Xi,Pi,SV,T,posiChannel)
N_state = size(Xi, 1);    

[Xp, ~] = ConstantVelocity_static(Xi, T);%1 状态预测值

[~, fy] = ConstantVelocity_static(Xp, T);%2 状态转移矩阵

[gXp, H] = obsEquation_static(Xp, SV);%3 观测方程：  gXp：预测观测值   H：观测矩阵
