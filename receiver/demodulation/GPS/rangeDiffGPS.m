%% start PVT
% initial
function [recv_time, ephemeris, pvtCalculator, config] = rangeDiffGPS(channels, config, recv_time, ephemeris, pvtCalculator) 
%% 初始化
...
%       calculate raw Pseudoranges        
      [prDiff, rawP, activeChannel, pvtCalculator] = calculatePseudorangesGPS_hatched...
      ...
      % Performing the PVT
       [xyzdt,~,~,~] = leastSq_rangeDiff(satPositions, prDiff, activeChannel, config.elevationMask, checkNGEO); %freqforcal,reveiver.recv_cfg);       
         ...
         %   采用单点定位授时
        [dt,el,az,dop] = leastSquarePos_GPS(satPositions, rawP+satClkCorr*299792458, ... 
            transmitTime-satClkCorr,ephemeris,activeChannel, config.elevationMask); %freqforcal,reveiver.recv_cfg);    
        queue=sort(el(2,:));
...
%        fclose(fid);
       ...
       rinexobsGPS_data(recv_time,rawP,inteDoppler,dopplerfre,CNR,dt(4),queue, logName);
       ...
       [pvtCalculator] = rinexnaviGPS_data(recv_time,ephAll,satClkCorr,queue, pvtCalculator,logName);
