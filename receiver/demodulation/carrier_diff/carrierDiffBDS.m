%% start PVT
% initial
function [recv_time, ephemeris, pvtCalculator, config] = carrierDiffBDS(channels, config, recv_time, ephemeris, pvtCalculator)
%% 初始化
...
for  n=1:config.numberOfChannels
    ...
    if  ~isnan(prnNum) && (ephemeris(prnNum).ephReady==1 || ephemeris(prnNum).updateReady == 1) ...
        ...
        % 积分多普勒值
        inteDoppler(activeChannel(2,svnum)) = -1*channels(n).CH_B1I(1).carrPhaseAccum;     % 波长=299792458/1561098000
        % 多普勒频移
    ...
    if svnum >=1 && n == config.numberOfChannels 
        ...
        % Performing the PVT
        [xyzdt,el,az,dop] = leastSquarePos(satPositions, rawP+satClkCorr*299792458, ...
            transmitTime-satClkCorr,ephemeris,activeChannel, config.elevationMask, checkNGEO); %freqforcal,reveiver.recv_cfg); ;
        queue = sort(el(2,:));
        [doubleDiff, rawP, activeChannel, pvtCalculator] = PseudorangesofcarrDiff...
            (transmitTime, rxTime, activeChannel, inteDoppler, pvtCalculator, el);
        [pvtCalculator] = saveMatrix(doubleDiff, pvtCalculator, satPositions(1:3,:), activeChannel);    % 保存当前历元的双差矩阵信息
        [positionXYZ, pvtCalculator] = carrDiffResult(pvtCalculator, activeChannel, checkNGEO);     % 求载波相位双差解
        %=== Convert to geodetic coordinates ==============================
         %-------save x,y,z to XYZ.txt-------------------------------------%
       logName = strcat(config.logFilePath, pvtCalculator.logName, '_XYZ.txt');
       fid = fopen(logName, 'a');
       fprintf(fid,'%f%20f%20f\n',positionXYZ(1),positionXYZ(2),positionXYZ(3));
       fclose(fid);
       %-------printf results          ----------------------------------%
       [latitude,longitude,height] = cart2geo(positionXYZ(1),positionXYZ(2),positionXYZ(3),5);
       ...
       %% 修正本地时间误差
        ...
        config.truePosition = [positionXYZ(1), positionXYZ(2), positionXYZ(3)];
 ...
 %%  output log file
       ...
       [pvtCalculator] = rinexnavi_data(EphAll,satClkCorr,queue, pvtCalculator,logName);
       ...
       OutputGPGGA(latitude, longitude, height, BJhour, BJmin, BJsec, length(queue), config.logFilePath, pvtCalculator.logName);  % 输出GPGGA数据
        ...
        pvtCalculator.positionXYZ = positionXYZ(1:3);
