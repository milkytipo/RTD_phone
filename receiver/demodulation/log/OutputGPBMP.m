function OutputGPBMP(recv_time,channels,activeChannel, path, name)
%%
%输入参数
logName = strcat(path, name, '_GPBMP.txt');
...
for i=1:length(activeChannel(1,:))
    if length(channels(activeChannel(1,i)).CH_B1I) > 0  %  >0 则无多径也保存直达径信息
        num_mp=num_mp+1;
