function rinexmpGPS_data(recv_time,channels,activeChannel,logName)
%%
%输入参数
recorder_identifier = '>';
...
for i = 1 : length(activeChannel(1,:))
    if length(channels(activeChannel(1,i)).CH_L1CA)>1
        ...
        pathSum = pathSum + length(channels(activeChannel(1,i)).CH_L1CA);
    end
end
if isempty(queue)==0
    ...
    for ii=1:epoch_numofsat
        ...
        for iii=1:MP_num
                ...
                cnr=channels(queue_mp(ii,1)).CH_L1CA(iii).CN0_Estimator.CN0;       
                insertNum = channels(queue_mp(ii,1)).CH_L1CA(iii).preUnitNum + 1;    % C中从0开始计数
