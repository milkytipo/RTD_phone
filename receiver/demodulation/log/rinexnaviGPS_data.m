function [pvtCalculator] = rinexnaviGPS_data(recv_time,ephAll,satClkCorr,queue, pvtCalculator,logName)
satsys='G';
...
for i=1:length(queue)
    if pvtCalculator.logOutput.GPSephUpdate(queue(i)) == 1
        ...
        GPSweek=ephAll(queue(i)).eph.weekNumber;
        spare=0;
        ...
        fprintf(fid_1,'    %19.12E%19.12E%19.12E%19.12E\n',IDOT,spare,GPSweek,spare);
        
        pvtCalculator.logOutput.GPSephUpdate(queue(i)) = 0;
    end
end
fclose(fid_1);
fid_2=fopen(logNameIono,'at');
for i=1:length(queue)
    if pvtCalculator.logOutput.GPSionoUpdate(queue(i)) == 1
        ...
        pvtCalculator.logOutput.GPSionoUpdate(queue(i)) = 0;
    end
end
fclose(fid_2);
