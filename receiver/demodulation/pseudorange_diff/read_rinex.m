...
% filename='F:\wangyz\多径研究项目\sv_cadll\trunk\m\logfile\louding_2014-11-2_13-29-10.15O';
...
while 1     % this is the numeral '1'
    ...
    for ideci = 1 : decimate_factor
        ...
        for i = 1:numsvs(k)
            ...
            if strcmp(sys,'C')
                num_BDS = num_BDS + 1;
                ch.BDS(num_BDS,k) = str2double(line(2:3));
                SVID_MAT.BDS(ch.BDS(num_BDS,k),k) = 1;
                if numobs > 0
                    ob.BDS(ch.BDS(num_BDS,k),k,1) = str2double(line(4:17));
                end
                if numobs > 1
                    ob.BDS(ch.BDS(num_BDS,k),k,2) = str2double(line(20:33));
                end
                if numobs > 2
                    ob.BDS(ch.BDS(num_BDS,k),k,3) = str2double(line(36:49));
                end
                if numobs > 3
                    ob.BDS(ch.BDS(num_BDS,k),k,4) = str2double(line(52:65));
                end
            elseif strcmp(sys,'G')
                num_GPS = num_GPS + 1;
                ch.GPS(num_GPS,k)=str2double(line(2:3));
                SVID_MAT.GPS(ch.GPS(num_GPS,k),k) = 1;
                if numobs > 0
                    ob.GPS(ch.GPS(num_GPS,k),k,1) = str2double(line(4:17));
                end
                if numobs > 1
                    ob.GPS(ch.GPS(num_GPS,k),k,2) = str2double(line(20:33));
                end
                if numobs > 2
                    ob.GPS(ch.GPS(num_GPS,k),k,3) = str2double(line(36:49));
                end
                if numobs > 3
                    ob.GPS(ch.GPS(num_GPS,k),k,4) = str2double(line(52:65));
                end
            end
