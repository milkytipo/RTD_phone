...
%       Read Diff-information from RINEX with hatch_BD smoothing 
        [pr_base, Diff_time] = Range_corr;%输出差分量
...
%       calculate single difference between user and basestation
      [min_value, column]=min(abs(Diff_time - rxTime));%计算离接收机接收时间最近的时间
      for ii = 1:svnum
        pr_diff(activeChannel(2,ii)) = Pr_hatch(activeChannel(2,ii)) - pr_base(activeChannel(2,ii), column);%计算差分后的伪距
      end
