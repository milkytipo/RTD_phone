...
function [sat_position, el] = calSatPos2(SYST, refPos, refTime, almanac)
...
sat_position = -1 .* ones(maxPrnNo, 6);
el = -1 .* ones(1, maxPrnNo);

for prnNum = 1:maxPrnNo 

    if almanac.dect(prnNum) == 0    % 此处缺少卫星健康标识判断
        continue;
    end
        
    alm = almanac.alm(prnNum);
...
%     i_k = i0 + deltai;
    ...
    Z_k = y_k .* sin(i_k);
    sat_position(prnNum, 1:3) = [X_k, Y_k, Z_k];
    
    [~, el(prnNum), ~] = topocent(refPos, sat_position(prnNum, 1:3)' - refPos);
    ...
    switch SYST
        case 'BDS_B1I'
            if prnNum > 5
                ...
                sat_position(prnNum, 4:6) = [X_k1, Y_k1, Z_k1];
    %         [az, el, dist] = topocent(refPos, sat_position -refPos);
            else
                sat_position(prnNum, 4:6) = [0, 0, 0]; % GEO satellite
            end
