...
for channelNr = readyChnList
    ...
    index_a=find(trackResults(channelNr).skip<=sampleNum, 1, 'last' );
    index_b=find(trackResults(channelNr).skip>=sampleNum, 1, 'first' );
    x1=trackResults(channelNr).skip(index_a:index_b);
    y1=[index_a,index_b];
