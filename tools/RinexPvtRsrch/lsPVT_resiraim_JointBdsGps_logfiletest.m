...
function [pvtCalculator, recv_timer, satelliteTable_bds, satelliteTable_gps] = ...
    lsPVT_resiraim_JointBdsGps_logfiletest(satpos_bds, satpos_gps, obs_bds, obs_gps, transmitTime_bds, transmitTime_gps, satClkCorr_bds, satClkCorr_gps, ...
...
switch lspvt_raim_coder
    ...
    case 1 % sat_pvt_number>=6 & passing the raim checking
        if pvtCalculator.positionValid == -1 %首次定位
        ...
        pvtCalculator.clkErr(1,1) = pvtCalculator.clkErr(1,1) + cdtu(1);     % BDS: accumulated clk bias [meter]
        pvtCalculator.clkErr(1,2) = pvtCalculator.clkErr(1,2) + vel_xyz(4);  % BDS: accumulated clk drift freq [Hz]
        pvtCalculator.clkErr(2,1) = pvtCalculator.clkErr(2,1) + cdtu(2);     % GPS: accumulated clk bias [meter]
