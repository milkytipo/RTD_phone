...
doppBias_step = 0.0005;
doppBias_xvalues = -0.004:doppBias_step:0.004;
...
if plot_doppBias
    ...
    for j = 1 : length(doppBias)
        if doppBias(j)~= 0 && lifeTime(j)>0 % 去除小于5秒的数据
            if elelvation(j)>0 && elelvation(j)<90
                doppBias_index = ceil((doppBias(j))/doppBias_step) + addNum;
...
%     figure();
%     bar(doppbias_x_1, pool_doppBias_norm_1); 
    
%     dopp_sim_x = -0.3:0.001:0.3;
%     a1 = 0.06783;
%     b1 = -0.006248;
%     c1 = 0.07326;
%     mplt_mu_normal = 0; %mplt_pd_normal.mu;  6
%     mplt_sigma_normal = 0.05; %mplt_pd_normal.sigma;  5.8
%     doppbias_y = exp(-(dopp_sim_x-mplt_mu_normal).^2/2/mplt_sigma_normal^2)/mplt_sigma_normal/sqrt(2*3.1415927);
%     ratio_dopp = max(doppbias_y)/max(pool_doppBias_norm);
%     doppbias_y = doppbias_y / ratio_dopp;


%     dopp_sim_x_1 = 0:0.001:0.3;
%     a = 0.08974;
%     b = -19.88;     
%     for j = 1 : length(dopp_sim_x_1)
%         doppbias_y_1(j) = a*exp(b*dopp_sim_x_1(j));
%     end
%     hold on
%     plot(dopp_sim_x_1, doppbias_y_1)
%     
%     dopp_sim_x_2 = -0.3:0.001:0;
%     a = 0.08974;
%     b = 19.88;     
%     for j = 1 : length(dopp_sim_x_2)
%         doppbias_y_2(j) = a*exp(b*dopp_sim_x_2(j));
%     end
%     hold on
%     plot(dopp_sim_x_2, doppbias_y_2)
...
%     figure();
%     doppBias_values = histogram(doppBias, doppBias_xvalues);
end
