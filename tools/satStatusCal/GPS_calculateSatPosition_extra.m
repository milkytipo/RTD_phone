function [satPositions, satClkCorr] = GPS_calculateSatPosition_extra(transmitTime, eph, prn)
%星历参数
...
% 1List(1:activeCH_Num) = activeChannel(1:activeCH_Num);
...
toe=eph(1).toe;%星历参考时间
sqrtA=eph(1).sqrtA;%长半轴的平方根
e=eph(1).e;%偏心率
omega=eph(1).omega;%近地点幅角
deltan=eph(1).deltan;%卫星平均运动速率与计算值之差
M0=eph(1).M0;%参考时间的平近点角
omega0=eph(1).omega0;%按参考时间计算的升交点经度
omega_dot=eph(1).omegaDot;%OMEGA_DOT%升交点经度变化率
i0=eph(1).i0;%参考时间的轨道倾角
iDot=eph(1).iDot;%轨道倾角变化率
Cuc=eph(1).Cuc;%纬度幅角的余弦调和改正项的振幅
Cus=eph(1).Cus;%纬度幅角的正弦调和改正项的振幅
Crc=eph(1).Crc;%轨道半径的余弦调和改正项的振幅
Crs=eph(1).Crs;%轨道半径的正弦调和改正项的振幅
Cic=eph(1).Cic;%轨道倾角的余弦调和改正项的振幅
Cis=eph(1).Cis;%轨道倾角的正弦调和改正项的振幅

toc=eph(1).toc; %!!!!!!!!!!!!!!!!!causion
a0=eph(1).af0;
a1=eph(1).af1;
a2=eph(1).af2;
%transmitTime=eph(1).SOW;
TGD=eph(1).TGD;
%% find initial satellite clock correction
%%%%%%%%修正发射时间
dt = check_t(transmitTime(1)-toc);
% dt = check_t(transmitTime-toc);
%%%%%%计算卫星测距码相位时间偏差
satClkCorr(1,1) = a0+(a1+a2*dt)*dt-TGD;
%%%%%%%%%计算相对论校正项
%%%%%%%%%%%%%%%%计算信号发射时刻系统时间
time = transmitTime(1) - satClkCorr(1,1);
%% find sat position
%计算半长轴
A=sqrtA^2;
%时间校正
...
%总时间修正项
%%%%%%%%%%%进行一次反馈再次计算%%%%%%%%%%
satClkCorr(1,1)=a0+(a1+a2*dt)*dt+dtr-TGD;
time = transmitTime(1) - satClkCorr(1,1);
%时间校正
tk  = check_t(time - toe);
%计算卫星平均角速度
...
satPositions(4, 1) = X_k1;
satPositions(5, 1) = Y_k1;
satPositions(6, 1) = Z_k1;
% % else
% %   satPositions(4, 1) = 0;
% %   satPositions(5, 1) = 0;
% %   satPositions(6, 1) = 0;
...
satClkCorr(2,1) = a1 + 2*a2*dt + dtr_dot;
